...
// Секрет для подписи сессий. В проде это BOT_TOKEN. Если его нет
// (локальная разработка / тест-режим без бота) — падаем на SESSION_SECRET
// или дев-значение, чтобы сессии всё равно подписывались и проверялись.
// Проверка подписи Telegram-логина (verifyTelegramLogin) по-прежнему требует BOT_TOKEN.
const SESSION_SECRET = BOT_TOKEN || process.env.SESSION_SECRET || 'aurveda-dev-session-secret';
