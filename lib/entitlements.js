...
// Тест-доступ: только при TEST_LOGIN=1. Полный доступ получают синтетический
// test-admin и любые реальные Telegram-id из TEST_FULL_TG_IDS (через запятую).
// Работает даже без KV, чтобы кабинет можно было тестировать без хранилища.
function testFullAccess(tgId) {
  if (process.env.TEST_LOGIN !== '1' || !tgId) return false;
  if (String(tgId) === 'test-admin') return true;
  const ids = (process.env.TEST_FULL_TG_IDS || '')
    .split(',').map(s => s.trim()).filter(Boolean);
  return ids.includes(String(tgId));
}

export async function getEntitlements(tgId) {
  if (testFullAccess(tgId)) return { full: true, test: true, books: [] };
