...
/** Создать платёж. savePaymentMethod=true — для будущих автосписаний (подписка). */
export async function createPayment({ amountRub, description, returnUrl, metadata, savePaymentMethod }) {
...
/**
 * Авто-платёж по сохранённому методу (рекуррент, без участия пользователя).
 * Возвращает { id, status }. status 'succeeded' — деньги списаны.
 */
export async function chargeSavedMethod({ amountRub, description, paymentMethodId, metadata }) {
  if (!yooEnabled) throw new Error('YooKassa не настроена');
  const body = {
    amount: { value: Number(amountRub).toFixed(2), currency: 'RUB' },
    capture: true,
